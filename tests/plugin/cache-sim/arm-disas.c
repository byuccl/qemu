...
static const uint32_t COPROC_OPC1_MASK = CREATE_BIT_MASK(3) << 21;
static const uint16_t COPROC_OPC2_MASK = CREATE_BIT_MASK(3) << 5;
...
#define GET_COPROC_OPC2_BITS(bits) ((bits & COPROC_OPC2_MASK) >> 5)
#define GET_COPROC_OPC1_BITS(bits) ((bits & COPROC_OPC1_MASK) >> 21)
...
cp_load_store_e INSN_IS_COPROC_LOAD_STORE(insn_op_t* insn_data, uint32_t insn) {
    ...
    uint8_t op_bit = GET_LDST_B_BIT(insn);
    uint8_t op1_bits, cp_bits;

    if (arm_op1 == 0x6) {
        op1_bits = GET_COPROC_OP1_BITS(insn);
        // mask out bit 0
        ...
        if (cp_bits != 0xA) {
            // MCR is 10xxx0
            // MRC is 10xxx1
            if (op_bit && (op1_bits & 0x20)) {
                if (op1_bits & 0x01) {
                    returnVal = CP_MRC;                 /* A8-492 */
                } else {
                    returnVal = CP_MCR;                 /* A8-476 */
                }
            }
            // mask out bit 2
            // store is 0xxxx0 not 000x00
            //  load is 0xxxx1 not 000x01
            else if (op1_bits & 0x3B) {
    ...
    if (returnVal < CP_REG_TYPE_BASE) {
        ...
        // CRm
        insn_data->bitfield.Rm = GET_LDST_RM_BITS(insn);
        // opc1
        insn_data->bitfield.type = GET_COPROC_OPC1_BITS(insn);
        // opc2
        insn_data->bitfield.Rt2 = GET_COPROC_OPC2_BITS(insn);
    }
